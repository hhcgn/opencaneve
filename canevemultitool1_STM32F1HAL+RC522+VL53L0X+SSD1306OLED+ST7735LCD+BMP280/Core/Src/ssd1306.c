...
void ssd1306_UpdateScreen(I2C_HandleTypeDef *hi2c)
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        if(HAL_I2C_Mem_Write(hi2c, SSD1306_I2C_ADDR, 0x40, 1, &SSD1306_Buffer[SSD1306_WIDTH * i], SSD1306_WIDTH, 100)!=HAL_OK)  {
					uint64_t writeData[6];
					//itoa(deg,writeData,10);
					uint8_t i3=0,i4=0,str1[10],ass=0;
					//speed=6;
						if(deg<0) {ass=1;writeData[0]='-';deg=-deg;}
						else ass=0;
						while(deg!=0)
						{
							str1[i3]=numtoc(deg%10);
							deg/=10;
							i3++;
						}
						for(i4=0;i4<i3;i4++){
							writeData[i4+ass]=str1[i3-i4-1];
						}writeData[i3+ass]='\0';
						flashWriteData(124,writeData,6);
					__set_FAULTMASK(1);
					NVIC_SystemReset();
					//ssd1306_Init(hi2c);
					//HAL_Delay(10);
